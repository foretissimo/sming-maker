...
import FilterSection, { ARTISTS } from './components/FilterSection';
...
export default function App() {
  const [allSongs, setAllSongs] = useState(initialSongsData);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm sm:text-base font-bold text-emerald-300">
                포레스텔라 & 멤버별 음원총공 원클릭 스밍 도우미
              </h2>
              <p className="text-xs text-slate-400">
                멜론, 지니, 벅스 등 차트 반영 주기에 맞추어 최적의 1시간 리스트를 완성하고 앱으로 바로 담으세요.
              </p>
            </div>
          </div>
          <button
            onClick={() => setIsGuideOpen(true)}
            className="text-xs text-emerald-400 hover:text-emerald-300 underline font-medium cursor-pointer flex-shrink-0"
          >
            스밍 4계명 확인하기 &rarr;
          </button>
        </div>
